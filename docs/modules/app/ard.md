...
- Reason: measured before it was touched, a switch between two fixed tabs left the editor area, which is most of the screen, on the app's own centred "Loading" for 271ms in a production build and 367 to 441ms in dev, while the shell itself never moved: same DOM nodes for the sidebar, the tab bar and `<main>`, identical computed styles across 241 sampled frames, one navigation entry, no stylesheet churn.
  ...
  `key={id}` was not the cause: the App Router keys a dynamic segment by its parameter, so the page and everything it holds is discarded on every `/d/a` to `/d/b` with or without a key, which is why the state had to move to the layout and the page had to become empty.
  A cache would have made the second visit fast and the first no better, and would have shown a scene another device may have changed since.
- Debt created: none, though the surface now reads the address rather than receiving it as a prop, so a second route that shows a diagram would have to teach `openDiagramId` about itself.
- Revisit when: a route shows something other than one diagram in that surface, or React and Next give a page a way to survive a parameter change.
- Source: 0011_workspace_redesign

## 2026-09-20: the browser tab's title is set by the client, not by generateMetadata

- Decision: everything static in the head comes from the metadata API in the root layout; the diagram's name is applied at runtime by a client component in the `(editor)` layout, through the pure `pageTitle(name, appName)`.
- Alternatives rejected: `generateMetadata` on `/d/[id]`, which would have to read the item from DynamoDB; rendering a `<title>` element from a client component and letting React hoist it into the head.
- Reason: `generateMetadata` runs on the server, and no page here is allowed to read DynamoDB or S3, so the only place the name exists is the list the browser already fetched.
  Hoisting a second `<title>` would put two owners of one tag in the same head, one of them Next's.
  Setting `document.title` leaves the head entirely the metadata API's and follows the workspace, so a rename reaches the tab without a reload.
  The suffix stayed `my-napkin` rather than the shorter `napkin` of the phase's wording, because the product names itself the same way in the sidebar, on the login page and in both locales; the manifest carries the short form where a short form belongs.
- Debt created: none.
- Revisit when: a page outside the workspace needs a title of its own, or the product renames itself.
- Source: 0011_workspace_redesign

## 2026-09-20: the app's palette lives in four places, and only one of them is a token

- Decision: `src/lib/theme-colors.ts` holds the light and dark background as hex for `theme-color` and the manifest, `src/app/icon.svg` holds the plate and mark colours for both schemes, and `scripts/icons.mjs` holds the plate colour again for the icon it renders on an opaque background.
- Alternatives rejected: a `var(--background)` in the meta tag, which no browser resolves; reading the computed background at runtime and writing the meta from script, which is a hand-written head tag and a flash of the wrong colour; generating the three files from the CSS at build time.
- Reason: `<meta name="theme-color">`, a web manifest and a favicon are all consumed outside the document's CSS, so none of them can reach a token, and `docs/checks/styles.md` asks for something impossible here.
  What is possible is to keep the copies few, name them for the token they mirror, and make the drift fail: the e2e resolves the declared `theme-color` and the colour the page actually paints through a canvas and compares them channel by channel in both schemes.
  The icons have no such test; they are regenerated by hand from `icon.svg`.
- Debt created: retuning `--primary` or `--background` does not reach the favicon, the browser chrome or the manifest; the meta is caught by the test, the icons are not, and `scripts/icons.mjs` duplicates a colour from the file it reads.
- Revisit when: the palette is retuned, or a second brand colour appears outside the document.
- Source: 0011_workspace_redesign

## 2026-09-20: a control that is unavailable is aria-disabled, never disabled

- Decision: the rail's Libraries section carries `aria-disabled` and no click handler instead of the `disabled` attribute.
- Alternatives rejected: `disabled` with the tooltip on a wrapping element; hiding the section until the feature exists.
- Reason: a browser suppresses pointer events on a disabled control, so the tooltip that explains why it is unavailable never opens, and the keyboard cannot reach it either.
  The one thing the placeholder is for is saying "coming soon", so it has to be hoverable and focusable to do its job; it is announced as disabled and does nothing when clicked.
- Debt created: none.
- Revisit when: Libraries ships, or a second unavailable control appears and wants the same shape as a component.
- Source: 0011_workspace_redesign

## 2026-09-20: a global shortcut reads the address at the moment the key is pressed

- Decision: the tab shortcuts resolve the active diagram from `window.location.pathname` inside the handler, rather than from the `usePathname()` value the effect closed over.
- Alternatives rejected: keeping the closure and re-registering the listener on every change, which is what it did; a ref written during render.
- Reason: React can paint the new active tab before it flushes the effect that re-registers the listener, and the suite caught it: Alt+W in that window closed the tab the user had just left, with the bar already highlighting the new one.
  The router pushes the history entry before React commits, so the address is the freshest answer available to an event handler, and it is the same source the tab bar and the sidebar already treat as the truth about what is open.
- Debt created: none.
- Revisit when: the active diagram stops being derivable from the address.
- Source: 0011_workspace_redesign

## 2026-09-20: the e2e cleanup waits for the list before it counts, so green means the table is clean

- Decision: one `listReady` helper, waiting for the sidebar's loading skeleton to be gone, guards `openApp`, `newDiagram`, `goToRoot` and `removeItemsCreatedHere`; cleanup returns early when the spec tracked nothing and opens the rail when it did.
- Alternatives rejected: waiting for `item-list` to exist, which never happens in an empty folder or an empty workspace; deleting through the API from the test instead of through the UI.
- Reason: every count went through `item-list`, which does not exist while the list is loading, so a spec that ended right after a navigation counted zero of everything, skipped its own cleanup and passed, leaving rows in the dev table that Sebastian and every other spec share.
  It happened twice in this task before it was understood.
  The skeleton is in the server-rendered HTML, which is what makes waiting on it a real wait rather than an assertion that passes before React has started.
  Making it honest turned three silent passes into three timeouts, which was the fix working: those specs end on the login page or behind the rail, and now say so instead of pretending.
- Debt created: none.
...
- Decision: left as it is this phase, and written down instead.
- Alternatives rejected: moving the redirect to `useRouter().push` from a client boundary, which `src/lib/api.ts` does not have.
- Reason: it is the one ESLint warning the repository carries, and it cost the suite a flaky login case: a list request that 401s after a test clears its cookies navigates the page out from under the test.
  The fix is a real change to how every call reports an expired session, which does not belong in the last round of a polish phase.
