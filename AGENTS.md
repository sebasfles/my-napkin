...
- `infra/`: Terraform for AWS and GitHub (rulesets), two environments, applied by hand.
- `.github/workflows/`: CI on pull requests, deploy to dev on `develop`, promotion PR with e2e against dev, deploy to prd on `main`.
...
- Base branch: `develop`. `main` is production and only receives the promotion PR from `develop`.
